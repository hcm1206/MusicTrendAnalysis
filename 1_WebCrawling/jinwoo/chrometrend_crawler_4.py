...
batch_size = 5
for i in range(0, len(cleaned_artist_names), batch_size):
    ...
    try:
        ...
        # 날짜 열에서 시, 분, 초 제거 (오직 날짜만 유지)
        interest_over_time['date'] = interest_over_time['date'].dt.date
...
# 트렌드 데이터를 날짜를 기준으로 피벗하여 날짜를 열로 배치
pivot_trends = all_trends.pivot_table(index='변환한이름', columns='date', values='Score')

# 트렌드 데이터를 아티스트 데이터와 병합하기 위해 아티스트 기준으로 병합
final_data = pd.merge(artists, pivot_trends, how='left', left_on='변환한이름', right_index=True)

# 최종 데이터 저장
final_data.to_csv('artist_trends_5.csv', index=False)
